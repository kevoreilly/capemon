...
HOOKDEF(LONG, WINAPI, RegSetValueExA,
    ...
    if(ret == ERROR_SUCCESS) {
		if (g_config.dump_reg && cbData > 512) {
			CapeMetaData->DumpType = REGSETVAL;
			DumpMemory(lpData, cbData);
			DoOutputDebugString("RegSetValueExA hook: Dumped registry value %s.\n", lpValueName);
		}
...
HOOKDEF(LONG, WINAPI, RegSetValueExW,
    ...
    if(ret == ERROR_SUCCESS) {
		if (g_config.dump_reg && cbData > 512) {
			CapeMetaData->DumpType = REGSETVAL;
			DumpMemory(lpData, cbData);
			DoOutputDebugString("RegSetValueExW hook: Dumped registry value %s.\n", lpValueName);
		}
