...
public:
	ImportRebuilder(const CHAR * file) : PeParser(file, true)
	{
		pImportDescriptor = 0;
		pThunkData = 0;
		pImportByName = 0;
	...
	bool rebuildImportTable(const CHAR * newFilePath, std::map<DWORD_PTR, ImportModuleThunk> & moduleList, BOOL CapeFile);
