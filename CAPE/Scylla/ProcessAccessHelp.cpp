...
bool ProcessAccessHelp::openProcessHandle(DWORD dwPID)
{
	if (dwPID > 0)
	{
		if (hProcess)
		{
			...
			DebugOutput("openProcessHandle: There is already a process handle 0x%x", hProcess);
#endif
			return false;
		}
		else
		{
			...
			{
#ifdef DEBUG_COMMENTS
				DebugOutput("openProcessHandle: Failed to open handle, pid %d", dwPID);
#endif
				return false;
			}
		}
	...
	{
#ifdef DEBUG_COMMENTS
		DebugOutput("openProcessHandle: Wrong pid %d", dwPID);
#endif
		return false;
	}
...
HANDLE ProcessAccessHelp::NativeOpenProcess(DWORD dwDesiredAccess, DWORD dwProcessId)
{
	...
	cid.UniqueProcess = (HANDLE)(UINT_PTR)dwProcessId;
...
bool ProcessAccessHelp::readMemoryFromProcess(DWORD_PTR address, SIZE_T size, LPVOID dataBuffer)
{
	...
	if (returnValue)
	{
		...
		else
		{
			returnValue = true;
#ifdef DEBUG_COMMENTS
			DebugOutput("readMemoryFromProcess: ReadProcessMemory successfully read %d bytes", size);
#endif
		}
	}
...
/*	for (unsigned int i = 0; i < decodedInstructionsCount; i++) {
	...
	{
		//DebugOutput("disassembleMemory: res == DECRES_SUCCESS\n");
		return true;
	}
	else
	{
...
bool ProcessAccessHelp::readHeaderFromCurrentFile(const CHAR * filePath)
{
	return readHeaderFromFile(fileHeaderFromDisk, sizeof(fileHeaderFromDisk), filePath);
}

LONGLONG ProcessAccessHelp::getFileSize(const CHAR * filePath)
{
	LONGLONG fileSize = 0;
...
bool ProcessAccessHelp::writeMemoryToNewFile(const CHAR * file,DWORD size, LPCVOID dataBuffer)
{
...
bool ProcessAccessHelp::readHeaderFromFile(BYTE * buffer, DWORD bufferSize, const CHAR * filePath)
{
	DWORD lpNumberOfBytesRead = 0;
	LONGLONG fileSize = 0;
	DWORD dwSize = 0;
...
LPVOID ProcessAccessHelp::createFileMappingViewRead(const CHAR * filePath)
{
	return createFileMappingView(filePath, GENERIC_READ, PAGE_READONLY | SEC_IMAGE, FILE_MAP_READ);
}

LPVOID ProcessAccessHelp::createFileMappingViewFull(const CHAR * filePath)
{
	return createFileMappingView(filePath, GENERIC_ALL, PAGE_EXECUTE_READWRITE, FILE_MAP_ALL_ACCESS);
}

LPVOID ProcessAccessHelp::createFileMappingView(const CHAR * filePath, DWORD accessFile, DWORD flProtect, DWORD accessMap)
{
...
DWORD ProcessAccessHelp::getProcessByName(const CHAR * processName)
{
	DWORD dwPID = 0;
...
DWORD ProcessAccessHelp::getEntryPointFromFile(const CHAR * filePath)
{
...
bool ProcessAccessHelp::createBackupFile(const CHAR * filePath)
{
	size_t fileNameLength = strlen(filePath) + 5; //.bak + null
	BOOL retValue = 0;

	CHAR * backupFile = new CHAR[fileNameLength];

	strcpy_s(backupFile, fileNameLength, filePath);
	strcat_s(backupFile, fileNameLength, ".bak");
