...
public:
	...
	*/
	...
	{
		const CHAR* slash = strrchr(fullPath, L'\\');
		if(slash)
		{
			return slash+1;
		}
...
public:
	 ...
	 */
	static LPVOID createFileMappingViewRead(const CHAR * filePath);
	static LPVOID createFileMappingViewFull(const CHAR * filePath);

	/*
	 * Create a file mapping view of a file 
	 */
	static LPVOID createFileMappingView(const CHAR * filePath, DWORD accessFile, DWORD flProtect, DWORD accessMap);
	 ...
	 */
	static bool writeMemoryToNewFile(const CHAR * file,DWORD size, LPCVOID dataBuffer);
	 ...
	 */
	static bool readHeaderFromFile(BYTE * buffer, DWORD bufferSize, const CHAR * filePath);

	static bool readHeaderFromCurrentFile(const CHAR * filePath);
	...
	static LONGLONG getFileSize(const CHAR * filePath);

	static DWORD getEntryPointFromFile(const CHAR * filePath);

	static bool createBackupFile(const CHAR * filePath);
