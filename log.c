...
//
// The serialization state is per-thread: every hooked API call builds its
...
//     would read a PWSTR and write serializer state over the loader's string.
...
// s->init(). For BSON that is bson_init(), which zeroes the whole bson struct
// (a record the dead thread left half-built only leaks its buffer); for
// protobuf it is protobuf_init(), which zeroes the whole pb_ctx. istr_buf is
// scratch. Retained memory is bounded by the number of distinct thread ids
// that have logged, at sizeof(log_context_t) each (320 bytes on x64, 172 on
// x86), plus a ~100 KB pb_ctx for each of them that logged in protobuf mode.
...
protobuf_context_t *get_thread_pb_ctx(void)
{
	...
	if (!ctx)
		return NULL;
	if (!ctx->pb_ctx)
		ctx->pb_ctx = (protobuf_context_t *)calloc(1, sizeof(protobuf_context_t));
	return ctx->pb_ctx;
}

// g_bson and g_istr refer to a local named ctx, fetched once per function with
// get_log_context(). loq() gives up before serializing anything if that returns
// NULL, and entries are never removed, so every lookup made underneath loq()
// finds the entry loq() created. The serializer callbacks take no context
// argument, so each BSON callback below does its own lookup: appending a field
// costs two table walks, the log_* helper's and the callback's.
#define g_bson (ctx->bson_obj)
#define g_istr (ctx->istr_buf)
// The output format is process-wide: log_init() sets g_default_serializer
// before any hook can log, and a stream mixing BSON and protobuf frames is
// unsupported, so there is no per-thread copy to look up.
#define g_active_serializer (g_default_serializer)
...
// Positional, not designated: PlatformToolset v141 (VS2017) has no C99
// designated initializers in C mode. Order must track log_serializer_t.
